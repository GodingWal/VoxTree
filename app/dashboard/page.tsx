...
import { VoiceProgressBar } from "@/components/dashboard/voice-progress-bar";
      ...
      <header className="border-b bg-white/80 dark:bg-card/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="container flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/browse"
              className="text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:block lg:hidden"
            >
              Browse
            </Link>
            <DarkModeToggle />
            ...
            <div className="h-8 w-8 rounded-full bg-brand-sage/50 dark:bg-brand-green/20 flex items-center justify-center">
        ...
        <main className="container py-8 space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-xl bg-white dark:bg-card border p-5 flex items-center gap-4 shadow-sm">
              <div className="rounded-xl bg-brand-green/10 dark:bg-brand-green/20 p-3">
          ...
          <section className="space-y-2">
            <h3 className="text-sm font-medium text-brand-charcoal dark:text-foreground">
              Voice Slots
            </h3>
            <VoiceProgressBar
              used={voiceSlotsUsed}
                ...
                voices.map((voice) => (
                  <VoiceCard
                    ...
                    voice={{
                      ...
                      name: voice.name,
                      status: voice.status,
                      sample_url: voice.sample_url ?? null,
                    }}
                  />
                ))
              ) : (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {recentClips && recentClips.length > 0 ? (
                recentClips.map((clip) => (
                  ...
                  >
                    <div className="aspect-video bg-gradient-to-br from-brand-sage/30 to-brand-green/10 dark:from-brand-green/20 dark:to-brand-green/5 flex items-center justify-center">
                      <div className="h-12 w-12 rounded-full bg-white/80 dark:bg-card/80 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
          ...
          <section className="space-y-4">
            ...
            <ActivityFeed
              ...
              clips={(recentClips ?? []).map((c) => ({
                id: c.id,
                created_at: c.created_at,
                content_library: (c as Record<string, unknown>)
                  .content_library as { title?: string } | null,
                family_voices: (c as Record<string, unknown>)
                  .family_voices as { name?: string } | null,
              }))}
            />
          </section>
