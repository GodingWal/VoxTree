...
/* ─── Clone Type Tabs ─── */
type CloneTab = "voice" | "face" | "body";

const CLONE_TABS: { id: CloneTab; label: string; icon: string; available: boolean; description: string }[] = [
    { id: "voice", label: "Voice", icon: "🎤", available: true, description: "Clone your voice with AI" },
    { id: "face", label: "Face", icon: "👤", available: false, description: "Create AI face animations from a photo" },
    { id: "body", label: "Body", icon: "🧍", available: false, description: "Generate full body avatars and motions" },
];
...
export default function ClonePage() {
    const [activeTab, setActiveTab] = useState<CloneTab>("voice");
    ...
    useEffect(() => {
        if (analyserRef.current && wizardActive && analyserReady) drawWaveform();
        return () => { if (animationRef.current) cancelAnimationFrame(animationRef.current); };
    }, [analyserReady, wizardActive, drawWaveform]);

    /* ── Continuous audio level monitor (runs whenever mic is connected in wizard) ── */
    useEffect(() => {
        if (!analyserReady || !analyserRef.current || !wizardActive) return;
        ...
        const updateLevel = () => {
            ...
            let sum = 0;
            for (let i = 0; i < bufferLength; i++) { sum += freqData[i] * freqData[i]; }
    ...
    const testMicrophone = async () => {
        try {
            ...
            // Use the shared analyser if already connected, otherwise create a temporary one
            ...
            if (analyserRef.current && streamRef.current) {
                ...
                const source = tempCtx.createMediaStreamSource(tempStream);
            ...
            const testInterval = setInterval(() => {
                ...
                if (testCount >= 10) {
                    ...
                    // Only clean up temporary resources, not the shared ones
                    if (tempCtx) tempCtx.close();
                    if (tempStream) tempStream.getTracks().forEach((t) => t.stop());
                    // Restore the shared analyser's fftSize if we changed it
                    if (analyserRef.current && !tempCtx) analyserRef.current.fftSize = 2048;
            ...
            <main className="pt-16">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-3xl sm:text-4xl font-bold mb-2">Clone Studio</h1>
                        <p className="text-muted-foreground max-w-xl mx-auto">
                            Create AI clones of your voice, face, and body. Start with voice cloning today — more options coming soon.
                        </p>
                    </div>

                    {/* ─── Tab Bar ─── */}
                    <div className="mb-8">
                        <div className="flex items-center justify-center gap-2 p-1.5 rounded-xl bg-muted/50 backdrop-blur max-w-lg mx-auto">
                            {CLONE_TABS.map((tab) => (
                                <button
                                    key={tab.id}
                            ...
                            <Card className="border-0 shadow-xl bg-card/50 backdrop-blur overflow-hidden">
                                ...
                                <CardContent className="relative p-8 sm:p-12">
                                    <div className="text-center space-y-6">
                                        <div className="w-24 h-24 mx-auto rounded-full bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center ring-4 ring-primary/10">
                                        ...
                                        <div>
                                            ...
                                            <h2 className="text-2xl font-bold mb-3">
                                                {activeTab === "face" ? "Face Clone" : "Body Clone"}
                                            </h2>
                                            <p className="text-muted-foreground max-w-md mx-auto leading-relaxed">
                                                {activeTab === "face"
                                                    ? "Upload a photo and create AI-powered face animations. Bring characters to life with realistic facial expressions and lip-sync powered by your voice clone."
                                                    : "Generate full-body AI avatars with natural movements and gestures. Create personalized animated characters that look and move like your family members."}
                                            </p>
                                        </div>
                                            ...
                                            {activeTab === "face" ? (
                                                <>
                                                    <div className="p-3 rounded-lg bg-muted/50 text-center">
                                                        ...
                                                        <p className="text-xs text-muted-foreground">Simple one-photo setup</p>
                                                    </div>
                                                    <div className="p-3 rounded-lg bg-muted/50 text-center">
                                                        <p className="font-medium text-sm">🎭 Expressions</p>
                                                        <p className="text-xs text-muted-foreground">Realistic facial animation</p>
                                                    </div>
                                                    <div className="p-3 rounded-lg bg-muted/50 text-center">
                                                        <p className="font-medium text-sm">👄 Lip Sync</p>
                                                        <p className="text-xs text-muted-foreground">Syncs with voice clones</p>
                                                    </div>
                                                </>
                                            ) : (
                                                <>
                                                    <div className="p-3 rounded-lg bg-muted/50 text-center">
                                                        <p className="font-medium text-sm">🧍 Full Body</p>
                                                        <p className="text-xs text-muted-foreground">Realistic 3D avatars</p>
                                                    </div>
                                                    <div className="p-3 rounded-lg bg-muted/50 text-center">
                                                        <p className="font-medium text-sm">🏃 Motions</p>
                                                        <p className="text-xs text-muted-foreground">Natural body movements</p>
                                                    </div>
                                                    <div className="p-3 rounded-lg bg-muted/50 text-center">
                                                        <p className="font-medium text-sm">👕 Customizable</p>
                                                        <p className="text-xs text-muted-foreground">Outfits & appearance</p>
                                                    </div>
                                                </>
                                            )}
                                        </div>
                                        ...
                                        <div className="pt-4">
                                            <p className="text-sm text-muted-foreground">
                                                We&apos;ll notify you when {activeTab === "face" ? "Face" : "Body"} Clone is ready. In the meantime, try Voice Clone!
                                            </p>
                                            <Button
                        ...
                        <>
                            ...
                            {wizardActive ? (
                                <div className="space-y-3">
                                    ...
                                    {/* Current Phase: Script (left) + Controls (right) */}
                                ...
                                <div className="space-y-8">
                                    <div className="text-center">
                                        ...
                                        <h2 className="text-2xl sm:text-3xl font-bold mb-2">Create Your Voice Clone</h2>
                                        <p className="text-muted-foreground max-w-xl mx-auto">
