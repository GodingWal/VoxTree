...
2. **Secure Upload:** The VoxTree backend generates an AWS S3 Pre-signed URL. The audio file is uploaded directly and securely from the user's browser to our private S3 bucket.
3. **Database Entry:** A record is created in the `family_voices` Supabase table with a status of `processing`.
4. **ElevenLabs API Submission:** The backend securely fetches the audio from S3 and transmits it to the ElevenLabs "Add Voice" API endpoint using our secure API key.
...
2. **GPU Worker Hand-off:** Because RVC requires heavy GPU processing, the dataset is sent to an external GPU inference provider (e.g., Replicate or RunPod).
3. **Model Training:** The GPU worker trains a custom RVC model on the user's dataset. This process typically takes 15 to 30 minutes.
4. **Webhook Completion:** Once training is complete, the GPU worker sends a webhook back to VoxTree, updating the `rvc_model_id` and marking the `rvc_training_status` as `ready` in the database.
...
4. An FFmpeg background worker mixes the generated narration with the story's `instrumental_url` (background music/sound effects) and syncs it to the video.
...
2. The `isolated_vocals_url` and the user's `rvc_model_id` are sent to the external RVC GPU worker.
